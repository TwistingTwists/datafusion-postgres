...
fn tag(response: &Response) -> String {
    match response {
        Response::Execution(tag) => {
            pgwire::messages::response::CommandComplete::from(tag.clone()).tag
        }
...
#[tokio::test]
async fn test_sql_prepare_execute_deallocate() {
    ...
    assert!(client.portal_store().get_statement("my_stmt").is_some());
    assert_eq!(
        first_text_value(run_ok(&service, &mut client, "EXECUTE my_stmt").await).await,
        "42"
    );
    assert_eq!(
        first_text_value(run_ok(&service, &mut client, "EXECUTE my_stmt").await).await,
    ...
    assert_eq!(
        first_text_value(run_ok(&service, &mut client, "EXECUTE param_stmt(21)").await).await,
        "42"
    );
    assert_eq!(
        first_text_value(run_ok(&service, &mut client, "EXECUTE param_stmt(100)").await).await,
        "200"
    );

    assert_eq!(
        tag(&run_ok(&service, &mut client, "DEALLOCATE my_stmt").await),
        "DEALLOCATE"
    );
    assert!(client.portal_store().get_statement("my_stmt").is_none());
    assert_eq!(
        first_text_value(run_ok(&service, &mut client, "EXECUTE param_stmt(5)").await).await,
        "10"
    );

    assert_eq!(
        tag(&run_ok(&service, &mut client, "PREPARE stmt_a AS SELECT 1").await),
        "PREPARE"
    );
    assert_eq!(
        tag(&run_ok(&service, &mut client, "PREPARE stmt_b AS SELECT 2").await),
        "PREPARE"
    );
    assert_eq!(
        tag(&run_ok(&service, &mut client, "DEALLOCATE ALL").await),
        "DEALLOCATE"
    );
    assert!(client.portal_store().get_statement("param_stmt").is_none());
    assert!(client.portal_store().get_statement("stmt_a").is_none());
    assert!(client.portal_store().get_statement("stmt_b").is_none());
